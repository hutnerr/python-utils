...
class CloggerOverrideFactory:
    ...
    @staticmethod
    def clean() -> dict:
        """no timestamps, no source file, no tag, no colors. plain readable output."""
        return {
    ...
    @staticmethod
    def no_tag() -> dict:
        """hides the level tag (e.g. [INFO]) for this call."""
        return {
            CloggerSetting.USE_TAG: False,
        }
    ...
    @staticmethod
    def no_colors() -> dict:
        """strips all ANSI color codes for this call. useful when output will be read as plain text."""
        return {
            CloggerSetting.DISABLE_COLORS: True,
        }
    ...
    def print_output() -> dict:
        """strips all decorations for clean print-style output. useful when logging a
        final result meant to be read as plain text."""
        return {
            CloggerSetting.TIMESTAMPS_ENABLED: False,
            CloggerSetting.SHOW_SOURCE_FILE:   False,
            CloggerSetting.USE_TAG:            False,
